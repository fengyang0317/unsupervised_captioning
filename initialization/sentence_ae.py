...
tf.flags.DEFINE_string('job_dir', 'sen_ae', 'job dir')
  ...
  with tf.variable_scope('Discriminator'):
    ...
    if is_training:
      cell = tf.nn.rnn_cell.DropoutWrapper(cell, params.keep_prob,
                                           params.keep_prob)
    out, initial_state = tf.nn.dynamic_rnn(cell, noisy_sentence, lns,
                                           dtype=tf.float32)
...
def input_fn(batch_size, subset='train'):
  ...
  return {'noisy_sentence': key, 'len': lk}, {'sentence': sentence, 'len': ls}
